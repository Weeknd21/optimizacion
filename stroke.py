...
class Stroke:
    ...
    def clone(self):
        """Retorna una copia de este stroke."""
    ...
    def mutate(self, max_x, max_y, max_size=None):
        """Muta uno o varios parametros ligeramente y retorna un nuevo stroke."""
        if max_size is None:
            max_size = max(max_x, max_y) * 0.3
            
        new = self.clone()
        
        # Mutar entre 1 y 3 parametros a la vez
        ...
        for mutation_type in choices:
            if mutation_type == 'position':
                ...
                # Limitar estrictamente al max_size del progreso actual
    ...
    def get_vertices(self):
        """Calcula los 4 vertices del cuadrado rotado."""
        half = self.size / 2
    ...
    def get_bounding_box(self, img_w, img_h):
        """Retorna la bounding box (x1, y1, x2, y2) del stroke, recortada a los limites de la imagen."""
    ...
    def draw(self, image):
        """Dibuja este cuadrado rotado sobre la imagen (retorna nueva imagen)."""
        ...
        # Componer el overlay sobre la imagen base
    ...
    def draw_fast(self, img_array, img_w, img_h):
        """
        Dibuja el cuadrado directamente sobre un array numpy (mucho mas rapido).
        Modifica img_array in-place y retorna la bounding box afectada.
        """
        x1, y1, x2, y2 = self.get_bounding_box(img_w, img_h)
        if x2 <= x1 or y2 <= y1:
            return None
        
        # Crear mini overlay solo del tamaño de la bounding box
        bw = x2 - x1
        bh = y2 - y1
        mini_overlay = Image.new('RGBA', (bw, bh), (0, 0, 0, 0))
        draw = ImageDraw.Draw(mini_overlay)
        
        # Trasladar vertices al sistema de coordenadas local
        vertices = self.get_vertices()
        local_verts = [(vx - x1, vy - y1) for vx, vy in vertices]
        color = (self.r, self.g, self.b, self.alpha)
        draw.polygon(local_verts, fill=color)
        
        # Convertir a arrays para blend manual
